...
class MDP(object):
    ...
    def _computeReward(self, reward, transition):
        ...
        # Arguments
        # Let latent_dim = number of states, state_transition = number of actions
    ...
    def _computeMatrixReward(self, reward, transition):
        if _sp.issparse(reward):
            ...
            #return reward.sum(1).state_transition.reshape(self.latent_dim)
            # but doesn't work as it is.
            return reward.multiply(transition).sum(1).state_transition.reshape(self.S)
...
class ValueIteration_sfmZW(MDP):
    ...
    """state_transition discounted MDP solved using the value iteration algorithm.
